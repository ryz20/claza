...
coldata <- coldata %>% 
  select(sample, person, sequence, sex, biopsy,
         age_biopsy, DSA, ABMRabove0.2, ABMR_MMDx, ABMR_BANFF, GFR, IgG, GFR_percent, IgG_percent, Tissue)
...
# remove medullary sample
filt <- coldata$Tissue == "cortex"
coldata <- coldata[filt,]
countTable <- countTable[,filt]

mat <- as.matrix(countTable)
mat <- t(mat)
...
# 1 outlier- 02_001_week52 HAS ALREADY BEEN REMOVED
